...
class Forward_fashion(nn.Cell):
    def __init__(self, num_class=10):  # 一共分十类，图片通道数是1
        ...
        self.fc1 = nn.Dense(784, 256)  # 定义第一个全连接层，输入大小为784，输出大小为256
        self.relu1 = nn.ReLU()  # 定义第一个ReLU激活函数
        self.fc2 = nn.Dense(256, 128)  # 定义第二个全连接层，输入大小为256，输出大小为128
        self.relu2 = nn.ReLU()  # 定义第二个ReLU激活函数
        self.fc3 = nn.Dense(128, num_class)  # 定义第三个全连接层，输入大小为128，输出大小为num_class

    def construct(self, x):
        x = self.flatten(x)  # 展平输入数据
        x = self.fc1(x)  # 第一个全连接层
        x = self.relu1(x) # 第一个ReLU激活函数
        x = self.fc2(x)  # 第二个全连接层
        x = self.relu2(x) # 第二个ReLU激活函数

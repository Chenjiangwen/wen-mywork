...
IMAGE_SIZE = (50, 50)
check = [[1,2],[7,8],[3,4],[9,10],[13,14],[5,6],[17,18],[15,16],[19,20],[11,12]]
PCA_NUM = 2
...
def example(filename):
    ...
    for i in range(len(os.listdir(filename)) - 1):
        testimage = filename+str(i+1)+'.jpg'
        print(testimage)
        if recognize(testimage, eigenface, m, A) in check[i]:
            succeed += 1
    target = succeed/(len(os.listdir(filename)) - 1)
    print('准确率: {:.2%}'.format(target))

if __name__ == '__main__':
    example('data/TestDatabase/')
